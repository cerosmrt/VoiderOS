...
import shutil
import subprocess
from PyQt6.QtWidgets import QApplication, QMainWindow, QStackedWidget, QFileDialog, QWidget
from PyQt6.QtPrintSupport import QPrinter, QPrintDialog
from PyQt6.QtGui import QFont, QCursor, QShortcut, QKeySequence, QPainter, QColor
...
DEFAULT_CONFIG = {
    "void_dir": os.path.expanduser("~/Voider"),
    "book_dir": os.path.expanduser("~/Voider"),
    "active_file": os.path.expanduser("~/Voider/0.txt"),
    "void_key": "enter",
...
class StatusOverlay(QWidget):
    """TAB-toggled HUD: time, date, active file stats, battery, volume, current track."""
    ...
    def __init__(self, app):
        ...
        self._cache = {'battery': '—', 'volume': '—', 'track': '—'}
    ...
    def _reposition(self):
        w, h = self._app.width(), self._app.height()
        self.setGeometry(w - self._W - 20, h - self._H - 20, self._W, self._H)

    def _tick(self):
        self._refresh()

    def _refresh(self):
        self._cache['battery'] = self._battery()
        self._cache['volume']  = self._volume()
        self._cache['track']   = self._track()
        self.update()
    ...
    def _battery(self):
        for bat in ('BAT0', 'BAT1'):
            try:
                ...
                status = open(f'/sys/class/power_supply/{bat}/status').read().strip()
    ...
    def _volume(self):
        try:
            vol   = subprocess.run(['pamixer', '--get-volume'],
                                   capture_output=True, text=True, timeout=1).stdout.strip()
            muted = subprocess.run(['pamixer', '--get-mute'],
                                   capture_output=True, text=True, timeout=1).stdout.strip()
            return 'muted' if muted == 'true' else f"{vol}%"
        except Exception:
            return '—'
    ...
    def paintEvent(self, event):
        ...
        n_lines = len([l for l in app.line_ring.lines if l != '.'])

        rows = [
            ('time', now.strftime('%H:%M:%S')),
            ('date', now.strftime('%A  %d %B %Y').lower()),
            ('sep',  None),
            ('info', f'{fname}  ·  {n_lines} lines'),
            ('info', f'bat   {self._cache["battery"]}'),
            ('info', f'vol   {self._cache["volume"]}'),
            ('info', f'♪     {self._cache["track"]}'),
        ]
        ...
        fam   = app._app_font.family()
        f_big = QFont(fam, app._app_font.pointSize() + 6)
        f_sm  = QFont(fam, app._app_font.pointSize() - 1)
        ...
        for kind, text in rows:
            if kind == 'sep':
                ...
                p.drawLine(x, y + 6, self.width() - x, y + 6)
                y += 14
                continue
            if kind == 'time':
                p.setFont(f_big)
...
class FullscreenCircleApp(QMainWindow):
    ...
    def __init__(self, on_lock=None):
        ...
        # Resolve void_dir: config → XDG default, create if missing
        void_dir = self.config.get('void_dir', '') or os.path.expanduser("~/Voider")
        if not os.path.isdir(void_dir):
        ...
        self.entry.setFocus()
        self.entry.launchPressed.connect(self._launch_app)
        ...
        tab.setContext(Qt.ShortcutContext.ApplicationShortcut)
        tab.activated.connect(self._status_overlay.toggle)
    ...
    def switch_to_view(self, view_index):
        ...
        if hasattr(self, '_status_overlay') and self._status_overlay.isVisible():
    ...
    def _launch_app(self):
        """Ctrl+Enter: launch the entry text as a shell command."""
    ...
    def resizeEvent(self, event):
        ...
        if hasattr(self, '_fx_panel'):
            self._fx_panel.setGeometry(self.rect())
        if hasattr(self, '_status_overlay') and self._status_overlay.isVisible():
            self._status_overlay._reposition()
            self._status_overlay.raise_()
